...
use crate::view::{View, Event, Hub, Bus, Id, ID_FEEDER, RenderQueue, RenderData, THICKNESS_MEDIUM};
...
impl BottomBar {
    pub fn new(rect: Rectangle, current_page: usize, pages_count: usize, works_count: usize, maxlines: usize) -> BottomBar {
        ...
        let prev_rect = rect![rect.min.x, rect.max.y - side, rect.min.x + side, rect.max.y];
    ...
    pub fn update_works_label(&mut self, current_page: usize, works_count: usize, maxlines: usize, rq: &mut RenderQueue) {
        let works_label = self.children[1].as_mut().downcast_mut::<WorksLabel>().unwrap();
        works_label.update(current_page, works_count, maxlines, rq);
    }
    ...
    pub fn update_icons(&mut self, current_page: usize, pages_count: usize, rq: &mut RenderQueue) {
        ...
        if self.is_prev_disabled != is_prev_disabled {
            let index = 0;
            let prev_rect = *self.children[index].rect();
